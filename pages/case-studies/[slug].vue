...
const contentPath = computed(() => `/case-studies/${route.params.slug}`)

const { data: page } = await useAsyncData(
  () => `case-study:${route.path}`,
  () => queryCollection(collections.value.caseStudies).path(contentPath.value).first(),
  { watch: [locale] }
)
  ...
  <section class="section">
    <article class="article-shell">
      ...
      <div class="button-row" style="margin-top: 28px;">
        <NuxtLink :to="localePath('/case-studies')" class="button secondary">{{ t('labels.backToList') }}</NuxtLink>
      </div>
    </article>
  </section>
</template>
